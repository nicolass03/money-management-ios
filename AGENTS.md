...
- **Projections tab:** `ProjectionDisplayLogic.visibleRows` shows up to **2 past pay periods**, then the **current** period, then up to **10 future periods** (sorted by `payDate`). Older past periods are omitted. Header badge shows cumulative free from the last visible row. Each row displays **accumulated** (`cumulativeFree`) and **free** (`periodFree`) — not period spend. API horizon is `PROJECTION_MONTHS_FORWARD` (12 months) so monthly schedules can fill all 10 future slots.
...
- **Budgets tab:** expand a budget card to list expenses and add charges. Any valid expense date on write (no pay-period gate). No remaining-budget cap on write — tracking only. **Early pay** still requires paid date in the current pay period. **Active / History** segmented control; **Finish** calls `POST /budgets/:id/complete` (parity with web). Completed/history budgets: read-only (no edit, no `+ expense`). Projections use actual spent when `completedAt` is set.
